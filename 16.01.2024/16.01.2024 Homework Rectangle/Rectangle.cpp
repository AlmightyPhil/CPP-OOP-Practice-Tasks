...
{
	if (this->p1.getX() == this->p3.getX() && this->p2.getY() == this->p4.getY())
	{
		return true;
	}
	else
...
bool Rectangle::isSquare()
{
	if (isRectangle())
	{
		...
		if (calculateLength(p1.getX(), p2.getX())
			== calculateLength(p1.getY(), p3.getY()))
		{
			return true;
		}
...
{
	return second - first; // second is always greater in this case
}
